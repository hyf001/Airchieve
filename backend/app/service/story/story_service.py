...
async def list_stories(
    ...
    *,
    ...
    if theme_id is not None:
        rows = [story for story in rows if theme_id in (story.theme_ids or [])]
    if age_range_id is not None:
        rows = [story for story in rows if age_range_id in (story.age_range_ids or [])]
...
async def get_story(db: AsyncSession, story_id: int, user_id: int | None = None) -> StoryRead:
    ...
    return StoryRead(
        **_story_summary(story).model_dump(),
        body=story.body,
        narrative_style_id=story.narrative_style_id,
...
async def create_user_story(db: AsyncSession, user_id: int, payload: StoryCreate) -> StoryRead:
    story = Story(
        ...
        narrative_style_id=payload.narrative_style_id,
...
async def assert_story_usable(db: AsyncSession, user_id: int, story_id: int) -> StoryInternalDTO:
    ...
    return StoryInternalDTO(
        ...
        age_range_ids=story.age_range_ids or [],
        theme_ids=story.theme_ids or [],
        education_goal_ids=story.education_goal_ids or [],
...
async def start_creation_from_story(
    ...
    *,
    ...
    return StoryCreationSessionRead(
        ...
        prefilled_age_range_ids=story.age_range_ids,
        prefilled_theme_ids=story.theme_ids,
