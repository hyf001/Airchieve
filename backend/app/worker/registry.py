...
from app.service.story import run_story_generation_task
...
async def run_story_generation_task_by_owner(db: AsyncSession, task: GenerationTask) -> None:
    if task.owner_type == "story":
        await run_story_generation_task(db, task)
        return
    await run_story_task(db, task)
